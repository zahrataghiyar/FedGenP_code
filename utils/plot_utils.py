...
def load_results(args, algorithm, seed):
    ...
    hf = h5py.File(r"C:\Users\zahtag\Downloads\Phd\simulation\Federated\FedGen\results\{}.h5".format(alg), 'r')
    #hf = h5py.File(r"C:\Users\zahtag\Downloads\Phd\simulation\Federated\FedGen\results\cwrus2u10c4-alpha100_FedAvg_0.01_20u_32b_20_0.h5", 'r')
    metrics = {}
    for key in METRICS:
...
def plot_results(args, algorithms):
    ...
    plt.gcf()
    plt.legend(fontsize='10', title_fontsize='40', loc = 'lower right')
    #plt.grid()
    #plt.title(dataset_[0] + ' Test Accuracy', fontsize=10)
    ...
    fig_save_path = os.path.join('figs', dataset_[0] + '-' + dataset_[1] +'-' + dataset_[2] + '.png')
