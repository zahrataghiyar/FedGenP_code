...
from FLAlgorithms.trainmodel.generator import Generator, Generator_FedGenP
from utils.model_config import *
import os
os.chdir('C:\\Users\\zahtag\\Downloads\\Phd\\simulation\\Federated\\FedGen')
...
def get_data_dir(dataset):
    ...
    elif 'cwru' in dataset:
        dataset_=dataset.replace('alpha', '').split('-')
        alpha =dataset_[1]
        path_prefix=os.path.join('data', 'CWRU', 's2u10c4-alpha{}'.format(alpha))
        train_data_dir=os.path.join(path_prefix, 'train')
        test_data_dir=os.path.join(path_prefix, 'test')
        proxy_data_dir = 'data/proxy_data/CWRU/'
...
def create_generative_model(dataset, algorithm='', model='cnn', embedding=False):
    ...
    if 'FedGenP' in algorithm:
        ...
        if 'cnn' in algorithm:
            ...
            passed_dataset += '-cnn1'
        return Generator_FedGenP(passed_dataset, model=model, embedding=embedding, latent_layer_idx=-1, grl = False)

    elif 'FedGen' in algorithm:
        # temporary roundabout to figure out the sensitivity of the generator network & sampling size
        if 'cnn' in algorithm:
            gen_model = algorithm.split('-')[1]
            passed_dataset+='-' + gen_model
