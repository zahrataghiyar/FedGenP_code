...
class Generator_FedGenP(nn.Module):
    def __init__(self, dataset='mnist', model='cnn', embedding=False, latent_layer_idx=-1, grl = False):
        super(Generator_FedGenP, self).__init__()
    ...
    def build_network(self):
        ...
        self.representation_layer_mu = nn.Linear(self.fc_configs[-1], self.latent_dim)
        self.representation_layer_sigma = nn.Linear(self.fc_configs[-1], self.latent_dim)
    ...
    def reparameterize(self, mean, log_var):
        # Sampling the latent code from the distribution
        std = torch.exp(0.05 * log_var)  # Compute standard deviation from log variance
        epsilon = torch.randn_like(std)  # Sample from standard Gaussian distribution
        latent_code = mean + epsilon * std  # Reparameterization trick
        return latent_code
    def forward(self, labels, latent_layer_idx=-1, verbose=True):
        ...
        z = grad_reverse(z, self.grl)
        ### FC layers
        for layer in self.fc_layers:
            z = layer(z)
        mean = self.representation_layer_mu(z)
        log_var = self.representation_layer_mu(z)
...
class GradReverse(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, grl):
        ctx.save_for_backward(grl)
        return x.view_as(x)

    @staticmethod
    def backward(ctx, grad_output):
        grl, = ctx.saved_tensors
        if grl:
            return grad_output * -1, None
        else:
            return grad_output * 1, None
        
def grad_reverse(x, grl):
    return GradReverse.apply(x, grl)
