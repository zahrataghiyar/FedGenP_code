...
from FLAlgorithms.users.userFedGenP import UserFedGenP
...
class FedGenP(Server):
    def __init__(self, args, model, seed):
        ...
        for i in range(total_users):
            self.generative_models.append(create_generative_model(args.dataset, args.algorithm, self.model_name, args.embedding))        
        self.generative_models.append(create_generative_model(args.dataset, args.algorithm, self.model_name, args.embedding)) # the last one is for all users
        self.generative_model = create_generative_model(args.dataset, args.algorithm, self.model_name, args.embedding) #to be removed
        if not args.train:
        ...
        for i in range(total_users):
            self.generative_optimizers.append(torch.optim.Adam(
                params=self.generative_models[i].parameters(),
                lr=self.ensemble_lr, betas=(0.9, 0.999),
                eps=1e-08, weight_decay=self.weight_decay, amsgrad=False))
        self.generative_optimizers.append(torch.optim.Adam(
                params=self.generative_models[total_users].parameters(),
                lr=self.ensemble_lr, betas=(0.9, 0.999),
                eps=1e-08, weight_decay=self.weight_decay, amsgrad=False)) #this is for all users    

        self.generative_lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(
            optimizer=self.generative_optimizer, gamma=0.98)
        self.generative_lr_schedulers = []
        for i in range(total_users):
            self.generative_lr_schedulers.append(torch.optim.lr_scheduler.ExponentialLR(
            optimizer=self.generative_optimizers[i], gamma=0.98))        
        self.generative_lr_schedulers.append(torch.optim.lr_scheduler.ExponentialLR(
            optimizer=self.generative_optimizers[i], gamma=0.98)) #This is for all users
        ...
        for i in range(total_users):
            ...
            user=UserFedGenP(
                args, id, model, self.generative_models[i],self.generative_models[self.n_users], 
    ...
    def train(self, args):
        ...
        for glob_iter in range(self.num_glob_iters):
            ...
            for user_id1, user in zip(self.user_idxs, self.selected_users):
                verbose= user_id1 == chosen_verbose_user
                self.train_generator(user_id1, glob_iter,
                    self.batch_size,
    ...
    def train_generator(self, id, glob_iter, batch_size, epoches=1, latent_layer_idx=-1, verbose=False):
        """
        ...
        def update_generator_(user_id, glob_iter,  n_iters, student_model, TEACHER_LOSS, STUDENT_LOSS, DIVERSITY_LOSS):
            self.generative_models[user_id].train()
            student_model.eval()
            for i in range(n_iters):
                self.generative_optimizers[user_id].zero_grad()
                y=np.random.choice(self.qualified_labels, batch_size)
                y_input=torch.LongTensor(y)
                ## feed to generator
                gen_result=self.generative_models[user_id](y_input, latent_layer_idx=latent_layer_idx, verbose=True)
                ...
                diversity_loss = self.generative_models[user_id].diversity_loss(eps, gen_output)  # encourage different outputs
                ...
                for user_idx, user in enumerate(self.selected_users):
                    if user_id != user_idx:
                        ...
                        teacher_loss_=torch.mean( \
                            self.generative_models[user_id].crossentropy_loss(user_output_logp_, y_input) * \
                ...
                #self.generative_optimizers[user_id].step()                 
                
                for user_idx, user in enumerate(self.selected_users):
                    if user_id == user_idx:
                        self.generative_models[user_id].grl = True
                        user.model.eval()
                        weight=self.label_weights[y][:, user_idx].reshape(-1, 1)
                        expand_weight=np.tile(weight, (1, self.unique_labels))
                        user_result_given_gen=user.model(gen_output, start_layer_idx=latent_layer_idx, logit=True)
                        ...
                        teacher_loss = teacher_loss_
                        teacher_logit = user_result_given_gen['logit'] * torch.tensor(expand_weight, dtype=torch.float32)
                        self.generative_models[user_id].grl = False
                        ######### get student loss ############
                        student_output=student_model(gen_output, start_layer_idx=latent_layer_idx, logit=True)
                        student_loss=F.kl_div(F.log_softmax(student_output['logit'], dim=1), F.softmax(teacher_logit, dim=1))
                        if self.ensemble_beta > 0:
                            ...
                            loss = self.ensemble_alpha * self.n_users * teacher_loss #+ self.ensemble_eta * diversity_loss
                loss.backward() #retain_graph=True)
                self.generative_optimizers[user_id].step()
        ...
        for i in range(epoches):
            TEACHER_LOSS, STUDENT_LOSS, DIVERSITY_LOSS=update_generator_(id, glob_iter,
        ...
        if verbose:
            print(info)
        self.generative_lr_schedulers[id].step()
