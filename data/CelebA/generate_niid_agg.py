...
LOAD_PATH = 'C:\\Users\\zahtag\\Downloads\\Phd\\simulation\\Federated/CelebA_dataset/leaf-master/data/celeba/'
DUMP_PATH = './data/CelebA'
...
def process_data():
    ...
    def process_(mode, tf, ratio=0.1, user_lists=None, agg_user=-1):
        ...
        dump_path = f'{dump_path}/{mode}'
        if not os.path.exists(dump_path):
            os.makedirs(dump_path)

        #dump_path=os.path.join(dump_path, "{}.".format(mode) + args.format)



        dump_path = os.path.join(dump_path, '{}.pt'.format(mode))
